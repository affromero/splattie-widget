...
describe('GhostEffect', () => {
  it('offsets mesh position by amplitude', () => {
    ...
    const mesh = { position: { y: 0 }, rotation: { x: 0, y: 0, z: 0 } };
  ...
  it('respects phase offset', () => {
    const ghost = new GhostEffect();
    const mesh1 = { position: { y: 0 }, rotation: { x: 0, y: 0, z: 0 } };
    const mesh2 = { position: { y: 0 }, rotation: { x: 0, y: 0, z: 0 } };
  ...
  it('drifts yaw within bounds when driftYaw > 0', () => {
    const ghost = new GhostEffect();
    const mesh = { position: { y: 0 }, rotation: { x: 0, y: 0, z: 0 } };
    ghost.apply(mesh, { amplitude: 0.01, frequency: 1, wobble: 0, driftYaw: 0.02 }, 3);
    expect(mesh.rotation.y).not.toBe(0);
    expect(Math.abs(mesh.rotation.y)).toBeLessThanOrEqual(0.02);
  });

  it('does not drift yaw when driftYaw is unset', () => {
    const ghost = new GhostEffect();
    const mesh = { position: { y: 0 }, rotation: { x: 0, y: 0, z: 0 } };
    ghost.apply(mesh, { amplitude: 0.01, frequency: 1, wobble: 0.3 }, 3);
